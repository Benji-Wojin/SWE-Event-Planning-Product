...
(() => {
  ...
  function dependencyInfo(task, state=store.getState()) {
    ...
    return {links,ready:task.status==='blocked'&&links.length>0&&links.every(link=>resolved(link.task)),resolved};
  }
  function dependencyMarkup(task) {
    ...
    return `<section class="task-dependencies section-gap" aria-label="Linked tasks">
      ...
      ${info.links.length?`<h3>Waiting on</h3>${info.links.map(link=>{const t=link.task;if(!t)return '<p>Linked task unavailable. Ask the organizer to review it.</p>';const done=info.resolved(t);return `<article class="dependency-card"><div class="dependency-card-heading"><button class="text-button" data-action="task" data-id="${h(t.id)}">${h(t.title)}</button><span class="status-badge ${done?'status-done':'status-blocked'}">${done?'Complete':t.reportedAt&&t.requiresVerification&&!t.verifiedAt?'Awaiting verification':statuses[t.status]}</span></div><p class="muted">${h(member(t.owner).name)}</p>${done?`<p class="dependency-result">${h(t.completionResult?.text||t.note||'No result provided.')}</p><p class="muted">${h(t.verifiedAt?'Verified by '+member(t.verifiedBy).name:completionText(t))}</p>`:''}${manage&&task.status==='blocked'?`<button class="text-button dependency-unlink" data-action="unlink-dependency" data-id="${h(task.id)}" data-prerequisite="${h(t.id)}" aria-label="Unlink ${h(t.title)}">Unlink · keep task</button>`:''}</article>`;}).join('')}`:task.status==='blocked'?'<p>No follow-up task linked yet.</p>':''}
      ${task.status==='blocked'&&manage?`<details class="dependency-add"><summary>Link or add follow-up work</summary><form id="dependencyForm" data-id="${h(task.id)}"><label class="field">Existing task<select name="existingTask">${options([['','Create a new task'],...state.tasks.filter(t=>t.id!==task.id&&!info.links.some(link=>link.taskId===t.id)).map(t=>[t.id,t.title])],'')}</select></label><label class="field">New task name <span class="muted">If creating a task</span><input name="newTitle" maxlength="180" placeholder="What needs to happen first?"></label><div class="form-actions"><button class="btn btn-secondary" type="submit">Link follow-up</button></div></form></details>`:''}
  ...
  document.addEventListener('submit',async event=>{
    ...
    const taskForm=['responseForm','taskDetailForm','commentForm'].includes(form.id);
    ...
    try {
      ...
      if(form.id==='dependencyForm'){store.addDependency(form.dataset.id,data.existingTask?{taskId:data.existingTask}:{title:data.newTitle},ui.actor);render();taskDialog(form.dataset.id);toast('Follow-up linked.');}
