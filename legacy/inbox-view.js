  function inboxView(s) {
    ...
    return pageHeading('EMAIL, ORGANIZED BY TASK','One task. One conversation.','Replies collect here. Review the change, then update the task without leaving the conversation.',`<button class="btn btn-primary" data-action="paste-email">${icon('plus')} Add email</button>`)+`
  ...
  function changePreview(message,s,values=message.suggested) {
    ...
    return `<div class="change-summary-heading"><span class="eyebrow">${diff.isNew?'CREATE A TASK':'BEFORE YOU ACCEPT'}</span><h3>${diff.isNew?'This will create a new task':diff.changed.length?`${diff.changed.length} ${diff.changed.length===1?'field will':'fields will'} change`:'No task fields will change'}</h3><p>${diff.isNew?'A new task will be added using the details below.':`Only the highlighted fields change on “${h(diff.current?.title||'the selected task')}”.`}</p></div>
  ...
  function emailReader(m,s) {
    if(m.ignoredAt)return `<div class="review-result"><span class="eyebrow">SET ASIDE</span><h3>No task changes made</h3><p>This email stays in the conversation history. It will not change the plan.</p></div>`;
    if(m.appliedTaskId) {
      ...
      return `<div class="review-result"><span class="eyebrow">ACCEPTED UPDATE</span><h3>This update was accepted</h3><p>Recorded by ${h(member(m.appliedBy).name)} · ${timeLabel(m.appliedAt)}</p>${fields.length?`<table class="change-table"><thead><tr><th>Field</th><th>Before</th><th>Accepted</th></tr></thead><tbody>${fields.map(key=>`<tr><th scope="row">${changeNames[key]}</th><td>${m.before?changeValue(key,m.before[key]):'—'}</td><td>${changeValue(key,m.approved[key])}</td></tr>`).join('')}</tbody></table>`:'<p>This email was attached to the task without changing its fields.</p>'}<p class="unchanged-fields">This is the saved change at the time of acceptance. Later emails may have updated the task again.</p></div>`;
    }
    ...
    return `<form class="proposal-form" id="emailReviewForm" data-id="${h(m.id)}"><div id="changePreview" aria-live="polite">${changePreview(m,s)}</div>
      <div class="proposal-explanation"><strong>Why this suggestion?</strong><p>${h(p.reason||'The email contains an update for this task. Check the proposed fields against what the sender actually said.')}</p></div>
      ...
      <details class="proposal-edit" ${p.mode==='new'?'open':''}><summary>Edit proposal or choose a different task</summary><div class="form-grid">
      ...
      <p class="proposal-footnote">One email at a time. Nothing changes until you accept.</p></form>`;
  }
